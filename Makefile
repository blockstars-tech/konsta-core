...
build:
	...
	go build -o polygon-edge -ldflags="\
    	-X 'konsta.live/versioning.Version=$(LATEST_VERSION)' \
		-X 'konsta.live/versioning.Commit=$(COMMIT_HASH)'\
		-X 'konsta.live/versioning.Branch=$(BRANCH)'\
		-X 'konsta.live/versioning.BuildTime=$(TIME)'" \
	main.go
